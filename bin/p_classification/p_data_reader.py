...
class RcDataReader(object):
    ...
    def _get_feed_iterator(self, line, need_input=False, need_label=True):
        ...
        if len(sentence_emb_slot) > self._STD_SLOT_LENGTH:
            sentence_emb_slot = sentence_emb_slot[:self._STD_SLOT_LENGTH]
        elif len(sentence_emb_slot) < self._STD_SLOT_LENGTH:
            i = len(sentence_emb_slot)
            while i < self._STD_SLOT_LENGTH:
                sentence_emb_slot.append(self._PADDING_IDX)
                i += 1


        sentence_pos_slot = [self._feature_dict['postag_dict'].get(pos, self._UNK_IDX) for pos in sentence_pos_list]
        if len(sentence_pos_slot) > self._STD_SLOT_LENGTH:
            sentence_pos_slot = sentence_pos_slot[:self._STD_SLOT_LENGTH]
        elif len(sentence_pos_slot) < self._STD_SLOT_LENGTH:
            i = len(sentence_pos_slot)
            while i < self._STD_SLOT_LENGTH:
                sentence_pos_slot.append(self._PADDING_IDX)
                i += 1

        sentence_charemb_slot = [self._feature_dict['charemb_dict'].get(c, self._UNK_IDX) for c in sentence]
        if len(sentence_charemb_slot) > self._STD_SLOT_LENGTH:
            sentence_charemb_slot = sentence_charemb_slot[:self._STD_SLOT_LENGTH]
        elif len(sentence_charemb_slot) < self._STD_SLOT_LENGTH:
            i = len(sentence_charemb_slot)
            while i < self._STD_SLOT_LENGTH:
                sentence_charemb_slot.append(self._PADDING_IDX)
                i += 1
        ...
        if len(sentence_charemb_slot) == 0 or len(sentence_emb_slot) == 0 or len(sentence_pos_slot) == 0 \
                or len(label_slot) == 0:
            return None
